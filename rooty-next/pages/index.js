...
export default function Home() {
  ...
  const HandleStepsBackwards = (newStep) =>{
    setSteps(newStep)
    if(steps === 1){
      setButtonState(true);
      setLandingPage("")
  ...
  return (
    <Wrapper dir="column" bgColor="#FFFDFA" overflowX="hidden" overflowY="hidden">
      ...
      {buttonState === true && <FlexBox>
        <Button txt="Register" bgColor="#4F4DB0" onClick={()=>HandleButton("register")}></Button>
        <Button txt="Log In" bgColor="#4F4DB0" onClick={()=>HandleButton("logIn")}></Button>
        </FlexBox>}
      
      {landingPage === "register" && <FlexBox dir="column">
        <FlexBox width="100vw" height="30vh">
          <FlexBox bgImage="/back.png" onClick={()=>HandleStepsBackwards(Math.max(1, steps - 1))} width="40px" height="40px" position="absolute" top="40px" left="20px"></FlexBox>
            <AnimatePresence exitBeforeEnter>
              <motion.div key={headerImage} initial={{x: 1000}} animate={{x: 0, transition:{duration: 0.4, delay:0}}} exit={{x:-1000}}>
        ...
        <FlexBox width="100%" height="70vh" bgColor="#4F4DB0" borderRadius="15px 15px 0 0" justifyContent="flex-start" dir="column" padding="0px 0px 50px 0px" boxShadow="0px -4px 9px rgba(0, 0, 0, 0.25);">
          <AnimatePresence exitBeforeEnter>
            <motion.div key={headerText} initial={{x: 1000}} animate={{x: 0, transition:{duration: 0.4, delay:0}}} exit={{x:-1000}}>
          ...
          <AnimatePresence exitBeforeEnter>
            {steps === 1 && <FlexBox width="100%" maxWidth="900px" dir="column" alignItems="flex-start" justifyContent="flex-start" >
              <motion.div key={steps} initial={{x: 1000}} animate={{x: 0, transition:{duration: 0.4, delay:0.7}}} exit={{x:-1000}}>
      ...
      {landingPage === "logIn" && <FlexBox dir="column">
        <FlexBox width="100vw" height="30vh">
          <FlexBox bgImage="/back.png" onClick={()=>GoBack()} width="40px" height="40px" position="absolute" top="40px" left="20px"></FlexBox>
            <AnimatePresence exitBeforeEnter>
              <motion.div key={headerImage} initial={{x: 1000}} animate={{x: 0, transition:{duration: 0.4, delay:0}}} exit={{x:-1000}}>
                <ImgPlaceholder width="190px" height="190px" bgImage={headerImage}></ImgPlaceholder>
              </motion.div>
        ...
        <FlexBox width="100%" height="70vh" bgColor="#4F4DB0" borderRadius="15px 15px 0 0" justifyContent="flex-start" dir="column" padding="0px 0px 50px 0px" boxShadow="0px -4px 9px rgba(0, 0, 0, 0.25);">
          <AnimatePresence exitBeforeEnter>
            <motion.div key={headerText} initial={{x: 1000}} animate={{x: 0, transition:{duration: 0.4, delay:0}}} exit={{x:-1000}}>
              <Text padding='60px 20px 0px 20px' size='42px' weight='600' txt="Welcome Back" color="white" width="100vw" maxWidth="900px"></Text>
            </motion.div>
          </AnimatePresence>
          <AnimatePresence exitBeforeEnter>
            <FlexBox width="100%" maxWidth="900px" dir="column" alignItems="flex-start" justifyContent="flex-start" >
              <motion.div key={steps} initial={{x: 1000}} animate={{x: 0, transition:{duration: 0.4, delay:0.7}}} exit={{x:-1000}}>
                <Text padding='30px 0px 10px 20px' size='18px' weight='500' txt="BCIT Email" color="white" ></Text>
                <Input bgImage="/bx-envelope.svg" type="email" placeholder='ex/jdoe@my.bcit.ca' margin="0px 20px 0px 20px" padding='0 0 0 55px' width="90vw" maxWidth="900px" justifyContent="flex-start"></Input>
                <Text padding='30px 0px 10px 20px' size='18px' weight='500' txt="Password" color="white" ></Text>
          ...
          <FlexBox width='100vw' padding="0px 20px 0px 20px" position="absolute" bottom="30px">
            <Button type='next' txt="Log In" color="black" width="90vw" maxWidth="300px" onNext={()=>{r.push("/home")}} size="21px" fontWeight="700" padding="25px"></Button>
          </FlexBox>
        </FlexBox>
      }
